...
def trainer_preparation(model, learning_rate, epochs):
    ...
    spynet_params = [param for name, param in model.named_parameters() if 'spynet' in name]
    other_params = [param for name, param in model.named_parameters() if 'spynet' not in name]

    optimizer = optim.Adam([{'params': other_params, 'lr':learning_rate,'betas':(0.9, 0.99)}])
    

    scheduler = optim.lr_scheduler.MultiStepLR(optimizer,
                                               milestones=[int(epochs  / 2),
                                                           int(epochs * 8 / 10)],
                                               gamma=0.3)
